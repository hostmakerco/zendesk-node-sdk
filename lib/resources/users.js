module.exports = ({ api }) => {
  api.users = {
    ...
    listByOrganization: (organizationId, queryParams) =>
      api.request(`/organizations/${organizationId}/users.json`, { queryParams }),
    get: (id, queryParams) => api.request(`users/${id}.json`, { queryParams }),
    getMany: (queryParams) => api.request(`users/show_many.json`, { queryParams }),
